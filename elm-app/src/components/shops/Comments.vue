<template>
  <div class="comment" v-if="evaluation">
    ...
    <div class="shop-info">
      <!--  标签   -->
      <ul class="tags">
        <li :class="{'unsatisfied': tags.unsatisfied}" v-for="(tags, i) in evaluation.tags" :key="i">
          {{ tags.name }}
          <span v-if="tags.count > 0">{{ tags.count }}</span>
        </li>
      </ul>
      <!--   内容   -->
...
export default {
  ...
  methods: {
    getData () {
      this.$axios ('/api/profile/comments').then (res => this.evaluation = res.data)
    },
