<template>
  <div class="goods" v-if="shopInfo">
      ...
      <div class="foods-wrapper" ref="foodsScroll">
        <ul>
          <li class="food-list-hook" v-for="(item, index) in shopInfo.menu" :key="index">
            ...
            <div class="fooddetails" v-for="(foods, i) in item.foods" :key="i" @click="foodsHanle(foods)">
              <img :src="foods.image_path" alt="">
              <section class="fooddetails-info">
                <h4>{{ foods.name }}</h4>
                <p class="fooddetails-des">{{ foods.description }}</p>
                <p class="fooddetails-sales">
                  月售{{ foods.month_sales }}份 好评率{{ foods.satisfy_rate }}</p>
                <div class="fooddetails-price">
                  <span class="price">¥{{ foods.activity.fixed_price }}</span>
                  <cart-controll :food="foods"/>
                </div>
              </section>
            </div>
          </li>
    ...
    <!--  商品详情  -->
    <foods :food="selectedFood" :is-show="showFood" @close="close"/>
  </div>
</template>
...
import Foods from "@/components/shops/Foods";
...
export default {
    ...
    getData () {
      this.$axios ('/api/profile/batch_shop').then (res => {
        res.data.recommend.forEach (recommend => {
          recommend.items.forEach (item => item.count = 0)
        })
        res.data.menu.forEach (menu => {
          menu.foods.forEach (item => item.count = 0)
        })
  ...
  computed: {
    currentIndex () {
      for (let i = 0; i < this.listHigeht.length; i++) {
        let height1 = this.listHigeht[i];
        let height2 = this.listHigeht[i + 1];
