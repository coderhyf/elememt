        ...
        <div class="cartview-cartbody">
          ...
          <div class="entityList-cartbodyScroller">
            <ul class="entityList-cartlist">
              <li class="entityList-entityrow" v-for="(item, index) in selcectFoods" :key="index">
                <h4><span>{{ item.name }}</span></h4>
                <span class="entityList-entitytotal">{{ item.activity.fixed_price }}</span>
...
export default {
    ...
    isEmpty () {
      ...
      this.shopInfo.menu.forEach (menu => {
        menu.foods.forEach (foods => {
          if (foods.count) {
            empty = false;
            this.totalCount += foods.count;
            this.totalPrice += foods.activity.fixed_price * foods.count;
            this.selcectFoods.push (foods);
          }
        })
      })
      return empty
  ...
  methods: {
    clearData () {
      this.shopInfo.recommend.forEach (recommed => {
        recommed.items.forEach (item => item.count = 0)
      })
      this.shopInfo.menu.forEach (menu => {
        menu.foods.forEach (foods => foods.count = 0)
      })
    }
  }
}
